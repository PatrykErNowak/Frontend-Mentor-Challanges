import { FlagProps, TagProps, JobListItemProps } from './types';

export default function JobListItem({ onAddFilter, jobOffer }: JobListItemProps) {
  ...
  return (
    <li className={`relative pt-10 p-6 bg-white rounded-md shadow-xl lg:flex lg:gap-6 lg:pl-10 lg:p-6  ${featured && borderStyles}`}>
      ...
      <div className="lg:w-max">
        <div className="flex flex-wrap items-baseline gap-2 lg:gap-7">
          <p className="text-main font-bold">{company}</p>
          <div className="flex gap-1 lg:gap-2 text-xs md:text-sm font-bold">
        ...
        <p className="font-medium text-main-gray text-sm lg:text-base">
...
function Flag({ className, children }: FlagProps) {
...
function Tag({ onAddFilter, children }: TagProps) {
  return (
    <li
      onClick={() => onAddFilter(children)}
      className="relative flex justify-center items-center  font-bold px-3 py-1 text-main  rounded-md  overflow-hidden before:content-[''] before:size-full before:absolute before:bg-main before:opacity-15 hover:cursor-pointer hover:text-white hover:bg-main">
      {children}
    </li>
  );
}
